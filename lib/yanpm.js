...
Yanpm.prototype.install = function (config) {
    ...
    if (!this._pluginConfigs || !this._pluginConfigs.length) {
        ...
        deferred.resolve(); // resolve promise, so it completes right away
    ...
    return this._installPlugins()
        .then(function () {
            ...
            if(this._errors && this._errors.length) {
                deferred.reject(this._errors); // reject promise, so it completes right away
                return deferred.promise;
            }
...
Yanpm.prototype._getMainFile = function (pluginConfig) {
    try {
        var packageManifest = require(path.join(pluginConfig.dir, 'package.json'));
        // this._logger.log('packageManifest:', packageManifest);

        if(packageManifest.main) {
            return path.join(pluginConfig.dir, packageManifest.main);
        }
        else {
            return null;
        }
...
Yanpm.prototype._requirePlugin = function (pluginConfig) {
    ...
    try {
        ...
        try {
            // this._logger.log('could not load file from dir:', pluginConfig.dir);

            // sometimes require will try to load the wrong file :/
            var file = this._getMainFile(pluginConfig);
            // this._logger.log('MainFile:', file);
