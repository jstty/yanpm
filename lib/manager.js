...
Manager.prototype.load = function (cb) {
    if (!cb || !isFunction(cb)) {
        console.error("Load Callback needs to be a function");
    }
...
Manager.prototype._normalizeAdd = function (group, name, npmPackage, args) {
    ...
    if (isObject(group)) {
        ...
        if(pack.name.indexOf('@') > 0) {
            pack.name = pack.name.substr(0, pack.name.indexOf('@'));
        }
        //console.log("plugin:", JSON.stringify(pack, null, 2), ', group:', JSON.stringify(group, null, 2));
